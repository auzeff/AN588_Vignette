...
You may have wondered why we specified not to use `set.seed(1)` in your tree. It turns out that seed 1 actually doesn't have any extinction!
:::

------------------------------------------------------------------------

The labels are all well and good for a small tree, but in the case of our original 100-species model, it's way too messy to mean anything. Luckily, we can make it a bit simpler using some optional commands in the plot function (you might have noticed one of these in our solution above).
...
If repeat this process many many times, essentially creating a "random walk" along the parameter space, we will eventually get the posterior distribution!
...
We can create a function that takes this into account. We want a function that can take in the value of both of the parameters. It then needs to get their probability based on a specific probabiliity distribution - which in our example will be a normal distribution - and multiply them to get a **joint prior**.
...
prior_fn_normal <- function(par) { # par = c(lambda, mu)
  ...
  if (lambda > 0 && mu > 0 && mu < lambda) {
    return(dnorm(lambda, mean = 0.1, sd = 0.03) * dnorm(lambda, mean = 0.03, sd = 0.02)) # Multiply these distributions to join them
  } else {
...
prior_fn_exp <- function(par) {
  ...
  if (lambda > 0 && mu > 0 && mu < lambda) {
    return(dexp(lambda, rate = 10) * dexp(mu, rate = 10))
  } else {
    return(0)  # Do not allow impossible or illogical values
  }
}
