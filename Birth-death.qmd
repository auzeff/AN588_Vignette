...
-   **Speciation Rate (λ):** The rate at which new species emerge

-   **Extinction Rate (µ):** The rate at which species go extinct
...
Birth-death models rely on the **memoryless** property of Markov processes, where the probability of a future state depends only on the present state, not past events. Think of the board game Chutes and Ladders. In this game, a player’s movement is determined solely by their current position and number on the die that they roll. 
...
### **Likelihood Function in Birth-Death Models**

The likelihood function of a birth-death model estimates the probability of observing a given phylogenetic tree under the assumed rates of speciation and extinction.

For each branching event occurring at time ti, we define:

-   po(ti): Probability that a lineage goes extinct before time ti

-   po(ti): Probability that a lineage survives and splits by ti

These functions are derived from and allow for model-fitting to empirical phylogenetic data.
...
![](https://encrypted-tbn1.gstatic.com/images?q=tbn:ANd9GcR_9ovYoLmP0ADas8NUsg4jiePKLM0q1LTdBVkv_UEen9SOjirYCr-fJJNzprAo8S0epONvfSDEWvbMa_eKzwJkyg){width="271"}![](https://www.crittersquad.com/wp-content/uploads/2018/01/6670066023_99968c35e6_b.jpg){width="271"}
...
The two parameters for a simple constant-rate birth-death model are $λ$ and $µ$. λ represents the speciation rate in our simulated phylogeny, while µ represents the extinction rate. We'll start by making a tree using these parameters, as well as the maximum number of species ("max.taxa = ") we'd like our model to diversity to.
...
//saadams// Maybe we should show a smaller model of a tree as well (or have them build one) that could include species labels. That might be something people see in papers/would want to make so it might be interesting to include that as well :)
...
//saadams// I added this below for how we should think about likelihood functions (important for maximum likelihood stuff that follows) - let me know if you want to change this!
...
You might be wondering how we can be sure that a set of parameters are the best fitting of all the possibilities. You can imagine that a guess and check method would take a while and not be a reasonable way to apprach the problem. Luckily for us, the power of R will allow us to do this rather efficiently! Next, we will be looking at ways to estimate the parameters for a give tree with the maximum likelihood...
...
Earlier, we showed how we can compare the likelihood function for different parameter inputs in order to assess how well those given parameter values maximize the likelihood of observing the phylo data. We could make an algorithm that searches for those parameter values that maximize this likelihood score for phylo_small. This function would iteratively adjust the parameters in search of values that minimize the negative log-likelihood - which translates to maximizing the likelihood if you recall how log values work!

This optimization process adjusts parameters, beginning with the initial ones we provide, and then continuously updates the parameters in order to "improve" the likelihood. This process eventually converges on a final solution, which is the output we get in the form of a set of parameters. The process ceases (or "converges") when adjustments to the parameters no longer change/improve the likelihood significantly.

The following likelihood surface plot gives a rough sketch of which parameter values the algorithm may converge on when searching for the optimized parameters for modeling phylo_small. From this plot, you can get an idea of which values we might more or less expect to get from the algorithm when we finally perform our analysis. Plus, it's fun to look at!
...
surface_plot <- filled.contour(lambda_vals, mu_vals, lik_matrix,
...
surface_plot # see plot
```

Luckily, diversitree has a built-in function, find.mle, which abstracts this iterative optimization process. This allows us to find the parameters that maximize the likelihood for a certain tree very quickly. Let's take a look at how to use find.mle and other functions to perform a Maxlimum Likelihood (ML) analysis.

(1) Define our initial values

Defining good initial values, from which the algorithm will start, can help the optimization process run a bit better by speeding up convergence and avoiding local minima (which you can sometimes get stuck out). If the initial values are too far away from the true values, the optimization fails or gives unreliable results. So how do you decide good initial values? Typically, researchers won't know the speciation and extinction rates (0.2 and 0.05 respectively) like we do. There is no "right" way to go about defining these initial values, and there are plenty of methods that have been developed to decide this a bit more systematically. Often, it is the case that ecologists, by virtue of study and lot of time spent observing species and doing various analyses, have a pretty good idea of plausible starting points. Our initial guesses for lambda and mu will be 0.1 and 0.05, respectively. Since we have a pretty good idea of what these true values are already (given that we were the ones that defined them), we'll start off with a guess a slightly farther from the truth, while still being reasonable.
...
control = list(maxit = 10000) is a voluntary additional setting. Here, we say that 10,000 is the maximum number of iterations that this algorithm will be allow to perform. If the optimization doesn't converge within 10,000 iterations, then a warning will be issued and find.mle will stop running. Increasing the number of iterations with maxit gives the optimizer more chances to find the best parameter values.
...
**note:** df = 2 refers to the degrees of freedom. In this context, it really indicates the number of free parameters being estimated for the fitted model. In the case of the birth-death model, the parameters being estimated are:
: $λ$ (speciation/birth rate) 
: $µ$ (extinction/death rate)

Since there are 2 estimated parameters, df is equal to 2.
...
How does our birth-death model stacks up against other modeling methods? Here, we will compare our birth-death model, which accounts for extinction, with the Yule model or the "pure birth model." 

The Yule model assumes $mu$ = 0.

```{r}
lik.yule <- constrain(lik_small, mu ~ 0) # pure birth model (Yule) where the extinction rate = 0
...
Could adding an extinction component be a bad thing? 

Well, when you add more complexity to your model, you do run the risk of creating conditions that are not that useful when you try to use the model under for other circumstances. It's important to assess whether adding another parameter (death/extinction here) improves the model's accuracy significantly. We'll do this with a **likelihood ratio test**.

Let's use an ANOVA test to perform a **likelihood ratio test**:

```{r}
anova(fit_small, fit_yule)
```

A lower AIC (Akaike Information Criterion) indicates that the "full" model (fit_small) fits the data better than the Yule model (see significance codes for significance level). Briefly, in model selection AIC penalizes models that have a higher level of complexity (i.e. more parameters). Despite being more complex, the full model still fits the phylo_small tree better. Sometimes the trade-off between complexity and fit will favor the simpler model (the Yule model) over the more complex one (the birth-death model). In fact, some you might see this result.

Also notice that lnLik (the log likelihood) for the fit_small model is more positive than fit_yule. The AIC itself is based on the likelihood of the model so this should be expected. That is, the favorable model should have a higher likelihood value and a lower AIC. 

**note:** the AIC value is contextual and has no real meaning here outside of its utility for comparing the two models.
...
<https://www.stata.com/support/faqs/statistics/convergence-of-maximum-likelihood-estimators/>
<https://medium.com/@abhaysingh71711/maximum-likelihood-estimation-the-key-to-model-optimization-6b5f95b191d1>
<https://rdrr.io/cran/diversitree/man/find.mle.html>
<https://www.rdocumentation.org/packages/stats/versions/3.6.2/topics/nlm>
<https://bio.libretexts.org/Bookshelves/Evolutionary_Developmental_Biology/Phylogenetic_Comparative_Methods_(Harmon)/11%3A_Fitting_Birth-Death_Models/11.02%3A_Clade_Age_and_Diversity>
...
<https://api.rpubs.com/tomanderson_34/lrt#:~:text=Likelihood%20Ratio%20Tests&text=The%20LRT%20compares%20two%20hierarchically,your%20model%20significantly%20more%20accurate>
